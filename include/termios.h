...
/* 下面名称TC[*]的含义是tty 控制命令。取相应终端termios 结构中的信息(参见tcgetattr())。	*/
#define TCGETS		0x5401	/* 设置相应终端termios 结构中的信息(参见tcsetattr()，TCSANOW)。	*/
#define TCSETS		0x5402	/* 在设置终端termios 的信息之前，需要先等待输出队列中所有数据处理完(耗尽)。对于修改	*/
							/* 参数会影响输出的情况，就需要使用这种形式(参见tcsetattr()，TCSADRAIN 选项)。	*/
#define TCSETSW		0x5403	/* 在设置termios 的信息之前，需要先等待输出队列中所有数据处理完，并且	*/
							/* 刷新(清空)输入队列。再设置（参见tcsetattr()，TCSAFLUSH 选项）。	*/
#define TCSETSF		0x5404	/* 取相应终端termio 结构中的信息(参见tcgetattr())。	*/
#define TCGETA		0x5405	/* 设置相应终端termio 结构中的信息(参见tcsetattr()，TCSANOW 选项)。	*/
#define TCSETA		0x5406	/* 在设置终端termio 的信息之前，需要先等待输出队列中所有数据处理完(耗尽)。对于修改参数	*/
							/* 会影响输出的情况，就需要使用这种形式(参见tcsetattr()，TCSADRAIN 选项)。	*/
#define TCSETAW		0x5407	/* 在设置termio 的信息之前，需要先等待输出队列中所有数据处理完，并且刷新(清空)输入队列。	*/
							/* 再设置（参见tcsetattr()，TCSAFLUSH 选项）。	*/
#define TCSETAF		0x5408	/* 等待输出队列处理完毕(空)，如果参数值是0，则发送一个break（参见tcsendbreak()，tcdrain()）。	*/
#define TCSBRK		0x5409	/* 开始/停止控制。如果参数值是0，则挂起输出；如果是1，则重新开启挂起的输出；如果是2，	*/
							/* 则挂起输入；如果是3，则重新开启挂起的输入（参见tcflow()）。	*/
#define TCXONC		0x540A	/*刷新已写输出但还没发送或已收但还没有读数据。如果参数是0，则刷新(清空)输入队列；	*/
							/* 如果是1，则刷新输出队列；如果是2，则刷新输入和输出队列（参见tcflush()）。	*/
#define TCFLSH		0x540B	/* 下面名称TIOC[*]的含义是tty 输入输出控制命令。设置终端串行线路专用模式。	*/
#define TIOCEXCL	0x540C	/* 复位终端串行线路专用模式。	*/
#define TIOCNXCL	0x540D	/* 设置tty 为控制终端。(TIOCNOTTY - 禁止tty 为控制终端)。	*/
#define TIOCSCTTY	0x540E	/* 读取指定终端设备进程的组id(参见tcgetpgrp())。	*/
#define TIOCGPGRP	0x540F	/* 设置指定终端设备进程的组id(参见tcsetpgrp())。	*/
#define TIOCSPGRP	0x5410	/* 返回输出队列中还未送出的字符数。	*/
#define TIOCOUTQ	0x5411	/* 模拟终端输入。该命令以一个指向字符的指针作为参数，并假装该字符是在终端上键入的。用户必须	*/
							/* 在该控制终端上具有超级用户权限或具有读许可权限。	*/
#define TIOCSTI		0x5412	/* 读取终端设备窗口大小信息（参见winsize 结构）。	*/
#define TIOCGWINSZ	0x5413	/* 设置终端设备窗口大小信息（参见winsize 结构）。	*/
#define TIOCSWINSZ	0x5414	/* 返回modem 状态控制引线的当前状态比特位标志集（参见下面185-196 行）。	*/
#define TIOCMGET	0x5415	/* 设置单个modem 状态控制引线的状态(true 或false)(Individual control line Set)。	*/
#define TIOCMBIS	0x5416	/* 复位单个modem 状态控制引线的状态(Individual control line clear)。	*/
#define TIOCMBIC	0x5417	/* 设置modem 状态引线的状态。如果某一比特位置位，则modem 对应的状态引线将置为有效。	*/
#define TIOCMSET	0x5418	/* 读取软件载波检测标志(1 - 开启；0 - 关闭)。	*/
							/* 对于本地连接的终端或其它设备，软件载波标志是开启的，对于使用modem 线路的终端或设备则	*/
							/* 是关闭的。为了能使用这两个ioctl 调用，tty 线路应该是以O_NDELAY 方式打开的，这样open()	*/
							/* 就不会等待载波。	*/
#define TIOCGSOFTCAR 0x5419	/* 设置软件载波检测标志(1 - 开启；0 - 关闭)。	*/
#define TIOCSSOFTCAR 0x541A	/* 返回输入队列中还未取走字符的数目。	*/
#define TIOCINQ 0x541B		/* 窗口大小(Window size)属性结构。在窗口环境中可用于基于屏幕的应用程序。	*/
 ...
 * 程序可以更改这个数组中的值。如果定义了—POSIX—VDISABLE(\0)，那么当数组某一项值
 * 等于—POSIX—VDISABLE的值时，表示禁止使用数组中相应的特殊字符。
*/
...
extern speed_t cfgetispeed (struct termios *termios_p);	/* 返回termios_p 所指termios 结构中的接收波特率。	*/
extern speed_t cfgetospeed (struct termios *termios_p);	/* 返回termios_p 所指termios 结构中的发送波特率。	*/
extern int cfsetispeed (struct termios *termios_p, speed_t speed); /* 将termios_p 所指termios 结构中的接收波特率设置为speed。	*/
extern int cfsetospeed (struct termios *termios_p, speed_t speed); /* 将termios_p 所指termios 结构中的发送波特率设置为speed。	*/
extern int tcdrain (int fildes);						/* 等待fildes 所指对象已写输出数据被传送出去。	*/
extern int tcflow (int fildes, int action);				/* 挂起/重启fildes 所指对象数据的接收和发送。	*/
extern int tcflush (int fildes, int queue_selector);	/* 丢弃fildes 指定对象所有已写但还没传送以及所有已收到但还没有读取的数据。	*/
extern int tcgetattr (int fildes, struct termios *termios_p);	/* 获取与句柄fildes 对应对象的参数，并将其保存在termios_p 所指的地方。	*/
extern int tcsendbreak (int fildes, int duration);		/* 如果终端使用异步串行数据传输，则在一定时间内连续传输一系列0 值比特位。	*/
extern int tcsetattr (int fildes, int optional_actions,	/* 使用termios 结构指针termios_p 所指的数据，设置与终端相关的参数。	*/
