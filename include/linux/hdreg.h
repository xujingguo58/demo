...
struct partition
{
  ...
  unsigned char sys_ind;			/* 分区类型字节。0x0b-D0S; 0x80-0ld Minix; 0x83-Linux ...	*/
  unsigned char end_head;			/* 分区的结束磁头号。	*/
  unsigned char end_sector;			/* 结束扇区号(位0-5)和结束柱面号高2位(位6-7)。	*/
  unsigned char end_cyl;			/* 结束柱面号低8位。	*/
  unsigned int start_sect;			/* 分区起始物理扇区号。starting sector counting from 0 */
  unsigned int nr_sects;			/* 分区占用的扇区数。nr of sectors in partition */
};
