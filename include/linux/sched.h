...
*/
/*
* 在GDT表中寻找第1 个TSS 在全局表中的入口。0-没有用nul，1-代码段cs，2-数据段ds，3-系统段syscall
...
#define str(n) \
/* 将任务寄存器中TSS 段的有效地址——>ax	*/	__asm__("str %%ax\n\t" \
/* (eax - FIRST_TSS_ENTRY*8)——>eax	*/		"subl %2,%%eax\n\t" \
/* (eax/16)——>eax = 当前任务号。		*/		"shrl $4,%%eax" \
											:"=a" (n) \
 ...
 * 输入：%0 - 新TSS 的偏移地址(*&__tmp.a)；
 * 		%1 - 存放新TSS 的选择符值(*&__tmp.b)；
 * 		dx - 新任务n 的选择符；
 * 		ecx - 新任务指针task[n]。
 * 其中临时数据结构—tmp用于组建177行远跳转（far jump）指令的操作数。该操作数由4字节偏移
 * 地址和2字节的段选择符组成。因此—tmp中a的值是32位偏移值，而b的低2字节是新TSS段的
 * 选择符（高2字节不用）。跳转到TSS段选择符会造成任务切换到该TSS对应的进程。对于造成任务
 * 切换的长跳转，a值无用。177行上的内存间接跳转指令使用6字节操作数作为跳转目的地的长指针，
 * 其格式为：jmp 16位段选择符：32位偏移值。但在内存中操作数的表示顺序与这里正好相反。
 * 任务切换回来之后，在判断原任务上次执行是否使用过协处理器时，是通过将原任务指针与保存在
 * last—task—used—math变量中的上次使用过协处理器任务指针进行比较而作出的，参见文件
 * kernel/sched.c 中有关 math—state__restore()函数的说明。
 */
#define switch_to(n) {\
struct {long a,b;} __tmp; \
/* 任务n 是当前任务吗?(current ==task[n]?)	*/__asm__("cmpl %%ecx,_current\n\t" \
/* 是，则什么都不做，退出。					*/		"je 1f\n\t" \
/* 将新任务TSS的16位选择符——>__tmp.b。		*/		"movw %%dx,%1\n\t" \
/* current = task[n]；ecx =被切换出的任务。	*/		"xchgl %%ecx,_current\n\t" \
/* 执行长跳转至*&—tmp，造成任务切换。		*/		"ljmp %0\n\t" \
/* 在任务切换回来后才会继续执行下面的语句。		*/		\
/* 原任务上次使用过协处理器吗？				*/		"cmpl %%ecx,_last_task_used_math\n\t" \
/* 没有则跳转，退出。						*/		"jne 1f\n\t" \
/* 原任务上次使用过协处理器，则清crO中的任		*/		"clts\n" \
/* 务切换务切换标志 TS。					*/		"1:" \
		::"m" (*&__tmp.a),"m" (*&__tmp.b), \
...
/* 基址base 低16 位(位15-0)——>[addr+2]。	*/	__asm__("movw %%dx,%0\n\t" \
/* edx中基址高16 位(位31-16)——>dx。		*/		"rorl $16,%%edx\n\t" \
/* 基址高16位中的低8位(位23-16)——>[addr+4]。*/		"movb %%dl,%1\n\t" \
/* 基址高16位中的高8位(位31-24)——>[addr+7]。*/		"movb %%dh,%2" \
		::"m" (*((addr)+2)), \
		"m" (*((addr)+4)), \
		"m" (*((addr)+7)), \
		"d" (base) \
...
/* %0 - 地址addr；	*/
/* %1 - 地址addr 偏移6 处；edx - 段长值limit。	*/
#define _set_limit(addr,limit) \
/* 段长limit 低16 位(位15-0)——>[addr]。		*/	__asm__("movw %%dx,%0\n\t" \
/* edx 中的段长高4 位(位19-16)——>dl。			*/		"rorl $16,%%edx\n\t" \
/* 取原[addr+6]字节——>dh，其中高4 位是些标志。	*/		"movb %1,%%dh\n\t" \
/* 清dh 的低4 位(将存放段长的位19-16)。			*/		"andb $0xf0,%%dh\n\t" \
/* 将原高4位标志和段长的高4位(位19-16)合成1字节，	*/		"orb %%dh,%%dl\n\t" \
/* 并放会[addr+6]处。							*/		"movb %%dl,%1" \
		::"m" (*(addr)), \
		"m" (*((addr)+6)), \
		"d" (limit) \
		:) 
...
/* edx - 存放基地址(__base)；	*/
/* %1 - 地址addr 偏移2；%2 - 地址addr 偏移4；	*/
/* %3 - addr 偏移7。	*/
#define _get_base(addr) ({\
unsigned long __base; \
/* 取[addr+7]处基址高16位的高8位(位31-24)—>dh。	*/	__asm__("movb %3,%%dh\n\t" \
/* 取[addr+4]处基址高16位的低8位(位23-16)—>dl。	*/		"movb %2,%%dl\n\t" \
/* 基地址高16 位移到edx 中高16 位处。			*/		"shll $16,%%edx\n\t" \
/* 取[addr+2]处基址低16 位(位15-0)——>dx。		*/		"movw %1,%%dx" \
/* 从而edx 中含有32 位的段基地址。				*/		:"=d" (__base) \
		:"m" (*((addr)+2)), \
		"m" (*((addr)+4)), \
		"m" (*((addr)+7))); \
__base;})
