 ...
 * %0 - eax(__res)，%l - eax(__NR_##name)。其中name是系统调用的名称，与__NR__组
 * 合形成上面的系统调用符号常数，从而用来对系统调用表中函数指针寻址。
 * 返回：如果返回值大于等于0，则返回该值，否则置出错号eano，并返回-1。
 * 在宏定义中，若在两个标记符号之间有两个连续的井号’##’，则表示在宏替换时会把这两个标记
 * 符号连接在一起。例如下面第139行上的—NR—##name，在替换了参数name(例如是fork)之后，
 * 最后在程序中出现的将会是符号—NR—fork。参见《The C Programming Language》附录A.12.3。
 */
 ...
 * 等同于 gcc 的函数属性说明：void do_exit(int error—code) —attribute— ((noreturn));
 */
