...
#include <sys/types.h>	/* unistd.h needs this */	/* 供unistd.h使用 */
...
#define STRINGIFY(x) #x			/* 把x转换成字符串类型，用于出错显示语句中。用于出错时显示语句中表示扇区数。	*/
...
/* 如果程序命令行参数不是4或5个（执行程序名本身算作其中1个），则显示程序用法并退出。	*/
	if ((argc != 4) && (argc != 5))
		usage();
/* 如果参数是5 个，则说明带有根设备名。	*/
	if (argc == 5)
	{
/* 如果根设备名不是软盘("FLOPPY")，则取该设备文件的状态信息，若出错则显示信息，退出。	
 * 否则取该设备名状态结构中的主设备号和次设备号。如果根设备就是FLOPPY设备，则让主设备
 * 号和次设备号取0。表示根设备就是当前启动引导设备。*/
...
/* 接下来在标准错误终端上显示所选择的根设备主、次设备号。如果主设备号不等于2 (软盘)或
 * 3(硬盘)，也不等于0 (取系统默认根设备)，则显示出错信息并退出。
  */
