...
LD86	=ld86 -0 			# 是：-0 生成8086 目标程序；-a 生成与as --32 和ld -melf_i386 部分兼容的代码。

AS	=as --32 				# GNU 汇编编译器和连接器，见列表后的介绍。
LD	=ld -melf_i386
#LDFLAGS	=-x -M #-s 			# GNU 连接器ld -melf_i386 运行时用到的选项。含义是：-s 输出文件中省略所
...
# ? 公共符号如何放置；
# ? 连接中包含的所有文件成员及其引用的符号。
CC	=gcc -m32  $(RAMDISK) 		# gcc -m32  是GNU C 程序编译器。对于UNIX 类的脚本(script)程序而言，
...
CFLAGS	=-Wall -fleading-underscore -fno-stack-protector -O -g -fstrength-reduce -fomit-frame-pointer \
	 #-mstring-insns	# gcc -m32  的选项。前一行最后的'\'符号表示下一行是续行。
# 选项含义为：-Wall -fleading-underscore 打印所有警告信息；-O 对代码进行优化；
# -fstrength-reduce 优化循环语句；-mstring-insns 是
# Linus 自己为gcc -m32  增加的选项，可以去掉。
CPP	=cpp -nostdinc -Iinclude 	# cpp 是gcc -m32  的前(预)处理程序。-nostdinc -Iinclude 的含
...
#
ROOT_DEV=/dev/fd1 			# ROOT_DEV 指定在创建内核映像(image)文件时所使用的默认根文件系统所
...
# .c 文件编译生成.s 汇编程序。':'表示下面是该规则的命令。
	$(CC) $(CFLAGS) \
	-nostdinc -Iinclude -S -o $*.s $< 	# 指使gcc -m32  采用前面CFLAGS 所指定的选项以及
...
# 现该操作的具体命令。
	$(AS) -c -o $*.o $< 		# 使用as --32 编译器将汇编程序编译成.o 目标文件。-c 表示只编译
# 或汇编，但不进行连接操作。
.c.o: 			# 类似上面，*.c 文件-??*.o 目标文件。
	$(CC) $(CFLAGS) \
	-nostdinc -Iinclude -c -o $*.o $< 	# 使用gcc -m32  将C 语言文件编译成目标文件但不连接。
...
Image: boot/bootsect boot/setup tools/system tools/build
	...
	rm tools/kernel -f
	sync
	 				# 这两行是执行的命令。第一行表示使用tools 目录下的build 工具
...
disk: Image 				# 表示disk 这个目标要由Image 产生。
	dd bs=8192 if=Image of=/dev/fd0 # dd 为UNIX 标准命令：复制一个文件，根据选项
	sync;sync;sync
...
# ld -melf_i386 需要将连接映象重定向存放在System.map 文件中。
...
# main.c）执行gcc -m32  预处理操作，-M 标志告诉预处理程序输出描述每个目标文件相关性的规则，
