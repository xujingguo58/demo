...
! SYS-SIZE是要加载的系统模块长度，单位是节，16字节为1节。0x3000共为0x30000字节
! =196 kB (若以1024字节为1KB计，则应该是192KB)，对于当前的版本空间已足够了。 
! 这里感叹号'!'或分号表示程序注释语句的开始。
!
! 下面等号'='或符号'EQU'用于定义标识符或标号所代表的值，可称为符号常量。这个常量指明
! 编译连接后system模块的大小。这个等式"SYSSIZE = 0x3000"原来是由linux/Makefile中
! 第92行上的语句动态自动产生。但从Linux0.11版本开始就直接在这里给出了一个最大默认
! 值。原来参见的自动产生语句还没有被删除，参见程序5-1中第92行的说明。当该值为0x8000
! 时，表示内核最大为512KB。
...
!
! bootsect.s被bios-启动子程序加载至0x7c00 (31KB)处，并将自己 
! 移到了地址0x90000 (576KB)处，并跳转至那里。
! 它然后使用BIOS中断将'setup'直接加载到自己的后面(0x90200)(576. 5KB)，
! 并将system加载到地址0x10000处。
! 注意!目前的内核系统最大长度限制为(8*65536)(512KB)字节，即使是在 
! 将来这也应该没有问题的。我想让它保持简单明了。这样512KB的最大内核长度应该 
! 足够了，尤其是这里没有象minix中一样包含缓冲区高速缓冲。
! 加载程序已经做得够简单了，所以持续的读出错将导致死循环。只能手工重启。
! 只要可能，通过一次读取所有的扇区，加载过程可以做得很快。

! 伪指令（伪操作符）.globl或.global用于定义随后的标识符是外部的或全局的，并且即使不使用 
...
ROOT_DEV = 0x306 
! 设备号0x306指定根文件系统设备是第2个硬盘的第1个分区。当年Linux是在第2个硬盘上安装
! 了Linux 0.11系统，所以这里ROOT DI:V被设置为0x306。在编译这个内核时你可以根据自己根文件
! 系统所在设备位置修改这个设备号。这个设备号是!.roux系统老式的硬盘设备号命名方式，硬盘设备号
! 具体值的含义如下:
...
! 47-56行作用是将自身(bootsect)从日前段位置Ox07c0 (37IQ3)移动到0x9000 (6761I3)处，共256字
! (512字节)，然后跳转到移动后代码的go标号处，也即本程序的下一语句处。
...
! 实际上BIOS把引导扇区加载到0x7c00处并把执行权交给引导程序时，ss = 0x00'sp = 0xfffe。
...
! 由于代码段移动过了，所以要重新设置堆栈段的位置。 sp 只要指向远大于512 偏移（即地址0x90200）处都可以。
! 因为从0x90200 地址开始处还要放置setup 程序，而此时setup 程序大约为4 个扇区，因此sp 要指向大
...
load_setup:						! 68--77 行的用途是利用BIOS 中断INT 0x13 将setup 模块从磁盘第2 个扇区
								! 开始读到0x90200 开始处，共读4 个扇区。
								! 如果读出错，则复位驱动器，并重试，没有退路。
								! INT 0x13 的使用方法如下：
								! 读扇区：
								! ah = 0x02 - 读磁盘扇区到内存；
								! al = 需要读出的扇区数量；
								! ch = 磁道(柱面)号的低8 位；
								! cl = 开始扇区(0-5 位)，磁道号高2 位(6-7)；
								! dh = 磁头号；
								! dl = 驱动器号（如果是硬盘则要置位7）；
								! es:bx ?指向数据缓冲区；
								! 如果出错则CF 标志置位。
...
! 下面指令表示下一条语句的操作数在cs段寄存器所指的段中。它只影响其下一条语句。实际上，
! 由于本程序代码和数据都被设置处于同一个段中，即段寄存器cs和ds、es的值相同，因此本程序
! 中此处可以不使用该指令。
...
! 因为7*4 + 0 = 28，所以/dev/PSO (2，28)指的是1.44MB A驱动器，其设备号是0x021c 
! 同理/dev/atO (2，8)指的是1.2MBA驱动器，其设备号是0x0208。
! 下面root-dev定义在引导扇区508,509字节处，指根文件系统所在设备号。0x0306指第2 
...
!  (0xlD，0x02)。当编译内核时，你可以在Makefile文件中另行指定你自己的值，内核映像 
...
seg cs
	...
	cmp ax,#0
	jne root_defined
! 取上面第88行保存的每磁道扇区数。如果Sect0rS=15则说明是1.2MB的驱动器；如果 
! SeCtorS=18，则说明是1.44MB软驱。因为是可引导的驱动器，所以肯定是A驱。
	seg cs
	mov bx,sectors 		! 取上面第88 行保存的每磁道扇区数。
						! 如果sectors=15
						! 则说明是1.2Mb 的驱动器；
						! 如果sectors=18，则说明是
						! 1.44Mb 软驱。
