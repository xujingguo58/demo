...
/*
* 再次注意!!!这里己经处于32位运行模式，因此这里的$0x10并不是把地址0x10装入各个
* 段寄存器，它现在其实是全局段描述符表中的偏移值，或者更准确地说是一个描述符表项
* 的选择符。有关选择符的说明请参见setup.s中193行下的说明。这里$0x10的含义是请求
* 特权级0(位0-1}}、选择全局描述符表(位2}}、选择表中第2项(位3-15=2 )。它正好指
* 向表中的数据段描述符项。(描述符的具体数值参见前而setup一中212, 213行)
* 下而代码的含义是:设置ds, es, fs, g:为setup.s中构造的数据段(全局段描述符表第2项)
* 的选择符=0x10，并将堆栈放置在stack start指向的user stack数组区，然后使用本程序
* 后而定义的新中断描述符表和全局段描述表。新全局段描述表中初始内容与setup.s中的基本
* 一样，仅段限长从8Nf13修改成了16Nf13o  stack start定义在kernel/sched.c, 69行。它是指向
* user stack数组末端的一个长指针。第23行设置这里使用的栈，姑目‘称为系统栈。但在移动到
* 手l务0执行(init/main. c中137行)以后该栈就被用作任务0和任务1共同使用的用户栈了。
*/
...
* 出错。
* 针对该问题，目前内核中就在第25行之后添加了一条长跳转指令：'ljmp $(-KERNEL-CS)，$lf'，
...
*/
				 ...
				 * 设存在协处理器的情况下执行一个协处理器指令，如果出错的话则说明协处理器芯片不存在，
				 * 要设置CR0 中的协处理器仿真位EM（位2），并复位协处理器存在标志MP（位1）。
				 */
...
check_x87:
	...
	movl %eax,%cr0
	ret
.align 4 						/* 这里".align 4"的含义是指存储边界对齐调整。"2"表示调整到地址最后2 位为零，	*/
								/* 即按4 字节方式对齐内存地址。	*/
...
# 的指令在栈中压入了返回地址，而第140行则压入了mainO函数代码的地址。当head.s 
...
# 140行将main.c的地址压入堆栈，这样，在设置分页处理（setup—paging）结束后 
...
#主内存区空间由_模块管理。它涉及到页面映射操作。内核中所有其他函数就是这里指的一般 
#(普通)函数。若要使用主内存区的页面，就需要使用get—free—pageO等函数获取。因为主内 
...
#后面还预留了 252项的空间，用于放置所创建任务的局部描述符(LDT)和对应的任务状态段TSS
#的描述符。
