...
#include <unistd.h>
/* *.h 头文件所在的默认目录是include/，则在代码中就不用明确指明位置。如果不是UNIX
 * 的标准头文件，则需要指明所在的目录，并用双引号括住。 unistd.h标准符号常数与类型文件。
 * 其中定义了各种符号常数和类型，并申明了各种函数。如果还定义了__LIBRARY__，则还会包括
  * 系统调用号和内嵌汇编代码_syscall0()等。	*/
 ...
 * 直到一个执行execve 调用。
 * 这对堆栈可能带来问题。
 * 处理的方法是在fork()调用之后不让main()使用
 * 任何堆栈。
 * 因此就不能有函数调用 - 这意味着fork 也要使用内嵌的代码，否则我们在从fork()退出
 * 时就要使用堆栈了。
 *
 ...
 * 模式（到任务0）后执行内嵌方式的fork()和pauseO，因此可保证不使用任务0的用户栈。
 * 在执行moveto—user—mode()之后，本程序main()就以任务0的身份在运行了。而任务0是所 
 ...
 * 免弄乱堆栈。而在再次执行fork()并执行过execveO函数后，被加载程序已不属于内核空间，
 * 因此可以使用写时复制技术了。参见上一章“Linux内核使用内存的方法”一节内容。
 *
 * 下面—syscallOO是unistd.h中的内嵌宏代码。以嵌入汇编的形式调用Linux的系统调用中断 
 *  0x80。该中断是所有系统调用的入口。该条语句实际上是int fork()创建进程系统调用。可展 
 * 开看之就会立刻明白。syscallO名称中最后的0表示无参数，1表示1个参数。
 * 参见 include/unistd.h，133 行。
 */

inline _syscall0(int,fork)	

inline _syscall0(int,pause)	/* int pause()系统调用：暂停进程的执行，直到收到一个信号。	*/
inline _syscall1(int,setup,void *,BIOS)	/* int setup(void * BIOS)系统调用，仅用于
												 * linux 初始化（仅在这个程序中被调用）。	*/
inline _syscall0(int,sync)	/* int sync()系统调用：更新文件系统。	*/
#include <linux/tty.h>				/* tty 头文件，定义了有关tty_io，串行通信方面的参数、常数。	*/
#include <linux/sched.h>			/* 调度程序头文件，定义了任务结构task_struct、第1 个初始任务的数据。
 ...
 * 如果有 * 关于实时时钟更多的资料，那我很感兴趣。 这些都是试探出来的，以及看了
 * 一些bios 程序，呵！
 */


/* 这段宏读取CMOS实时时钟信息。outb—p和inb—p是include/asm/io.h中定义的端口输入输出宏。	*/
#define CMOS_READ(addr) ({ outb_p(0x80|addr,0x70); inb_p(0x71); })
/*
 * 这段宏读取CMOS 实时时钟信息。
 * 0x70 是写端口号，
 * 0x80|addr 是要读取的CMOS 内存地址。
 * 0x71 是读端口号。
*/
...
/* 该函数取CMOS实时钟信息作为开机时间，并保存到全局变量startup-time(秒)中。参见后面 
 *  CMOS内存列表说明。其中调用的函数kernel-mktime()用于计算从1970年1月1日0时起到 
 ...
 * 秒值发生了变化，那么就重新读取所有值。这样内核就能把与CMOS时间误差控制在1秒之内。	*/
	...
	do
	{
		...
		time.tm_mday = CMOS_READ(7);	/*  -月中的当天日期。	*/
...
/* 对于被创建的子进程，fork()将返回0 值，	对于原(父进程)将返回子进程的进程号。	*/
/* 所以180-184 句是子进程执行的内容。	*/
...
/* 下面是父进程执行的语句。	*/
/* wait()是等待子进程停止或终止，其返回值应是子进程的进程号(pid)。这三句的作用是父进程等待子进程的结束。	*/
/* &i 是存放返回状态信息的位置。 如果wait()返回值不等于子进程号，则继续等待。	*/
	if (pid>0)
		while (pid != wait(&i))
	/* nothing */;
 ...
 * /dev/tty0 作为stdin，并复制成stdout 和stderr。
 * 再次执行系统解释程序/bin/sh。但这次执行所选用的参数和环境数组另选了一套（见上面165-167 行）。
 * 然后父进程再次运行wait()等待。如果子进程又停止了执行，则在标准输出上显示出错信息“子进程pid 停
 * 止了运行，返回码是i”，然后继续重试下去…，形成“大”死循环。
 */
	while (1) {
		if ((pid=fork())<0) {
