...
#include <linux/sched.h>		/* 调度程序头文件。定义了任务结构task_struct、第1 个初始任务	*/
								/* 的数据。还有一些以宏的形式定义的有关描述符参数设置和获取的嵌入式汇编函数程序。	*/
...
#define _S(nr) (1<<((nr)-1))	/* 该宏取信号nr在信号位图中对应位的二进制数值。信号编号1-32。比如信	*/
								/* 号5的位图数值等于 // 1<<(5-1) = 16 = 00010000b	*/

#define _BLOCKABLE (~(_S(SIGKILL) | _S(SIGSTOP)))	/* 除了SIGKILL 和SIGSTOP 信号以外其它都是	*/
													/* 可阻塞的(…10111111111011111111b)。	*/

/* 显示任务号nr 内核调试函数。显示任务号nr的进程号、进程状态和内核堆栈空闲字节数（大约）。	*/
...
/* NR—TASKS是系统能容纳的最大进程(任务)数量(64个)，定义在include/kernel/sched.h第4行。	*/
void
show_stat (void)
{
	int i;
...
/* PC机8253定时芯片的输入时钟频率约为1.193180MH^Linux内核希望定时器发出中断的频率是
 ...
 * ss被设置为内核数据段选择符（0x10），指针esp指在user—stack数组最后一项后面。这是
...
/* 现在，last—task—used—math指向当前任务，以备当前任务被交换出去时使用。此时如果当前
...
/* 让tmp指向已经在等待队列上的任务(如果有的话)，例如inode->i—wait。并且将睡眠队列头
...
为不可中断的等待状态，并执行重新调度。	*/
	...
	*p = current;							/* 将睡眠队列头的等待指针指向当前任务。	*/
	...
	了该队列，那么当系统切换去执行头指针所指的进程A时，该进程才会继续执行163行，把
	队列后一个进程B置位就绪状态（唤醒）。而当轮到B进程执行时，它也才可能继续执行
	163行。若它后面还有等待的进程C，那么它也会把C唤醒等。这里在163行前还应该添加
...
{
	struct task_struct *tmp;
/* 若指针无效，则退出。（指针所指的对象可以是NULL，但指针本身不会为0）。如果当前任务是任务0，则死机（impossible!）。	*/
	if (!p)
		return;
...
新调度。	*/
	...
	/* 等待队列的任务均被抹掉了。当然，同时也需删除192行上的语句。参见图4.3。	*/
	*p = NULL;
	if (tmp)
		tmp->state = 0;
}
...
/* 下面数组分别存放各软驱马达启动所需要的滴答寸间为50个滴答(0.5秒）	*/
 ...
 * 局部变量selected是选中软驱标志（blk—drv/floppy.c，122）?mask是所选软驱对应的
...
取当前DOR寄存器值到临时变量mask中，并把指定软驱的马达启动标志置位。	*/
	...
	滴答）。若已经启动，则再设置启动定时为2个滴答，能满足下面do_floppy_timerO中先递
...
/* 时钟中断C函数处理程序，在system—call.s中的—timer—interrupt(176行)被调用。
...
递增；[Linus把内核程序统称为超级用户（supervisor）的程序，见system—call.s，193行
上的英文注释]。如果cpl > 0，则表示是一般用户程序在工作，增加utime。
*/
	if (cpl)
		current->utime++;
	else
	 ...
	 * 并将该处理程序指针置为空。然后去掉该项定时器。next—timer是定时器链表的头指针。	*/
	if (next_timer)
	...
	if (current_DOR & 0xf0)
		do_floppy_timer ();
/* 如果进程运行时间还没完，则退出。否则置当前任务运行计数值为0。并且若发生时钟中断时正在
...
符表寄存器Idtr中。注意！！是将GDT中相应LDT描述符的选择符加载到Idtr。只明确加
