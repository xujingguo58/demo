...
static void die(char * str,long esp_ptr,long nr)
{
	...
	printk("%s: %04x\n\r",str,nr&0xffff);
/*下行打印语句显示当前调用进程的CS:EIP、EFLAGS和SS:ESP的值。参照图8-4，这里esp[0]
  即为图中的espO位置。因此我们把这句拆分开来看为：
 (1) EIP:\t%04x:%p\n — esp[l]是段选择符（cs），esp[0]是eip
...
/* 这两个函数均是嵌入式汇编 宏程序(include/asm/system.h,第 36 行、39 行)。	*/
void trap_init(void)
{
	int i;
	...
	/* 下面将 int17-48 的陷阱门先均设置为reserved，以后每个硬件初始化时会重新设置自己的陷阱门。 	*/
	for (i=17;i<48;i++)
