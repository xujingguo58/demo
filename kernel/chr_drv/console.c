 ...
 * 该模块实现控制台输入输出功能
 * ’void con_init(void)’
 * 'void con_write(struct tty—queue 木 queue)，
 *希望这是一个非常完整的VT102实现。
 */
/*
...
/* 以下这些变量用于屏幕卷屏操作。（origin表示移动的虚拟窗口左上角原点内存地址）。	*/
 ...
 * II主机通过发送不带参数或参数是0的设备属性（DA）控制序列（’ESC [c’或’ESC [0c’ ]
 * 要求终端应答一个设备属性控制序列（ESC Z的作用与此相同），终端则发送以下序列来响应
 * 主机。该序列（即’ESC [?l;2c’]表示终端是高级视频终端。
*/
...
字节表示)。再选择显示控制数据寄存器rl3，然后写入卷屏起始地址低字节。向右移动1位
表示除以2，同样代表屏幕上1个字符用2字节表示。输出值是相对于默认显示内存起始位置
video_mem_start操作的，例如对于彩色模式，viedo_mem_start =物理内存地址0xb8000。
*/
	cli();
	outb_p(12, video_port_reg);		/* 选择数据寄存器rl2，输出卷屏起始位置高字节。	*/
	outb_p(0xff&((origin-video_mem_start)>>9), video_port_val);
	outb_p(13, video_port_reg);		/* 选择数据寄存器rl3，输出卷屏起始位置低字节。	*/
...
而MDA单色显示卡只能进行整屏滚屏操作。该函数对EGA和MDA显示类型进行分别处理。如果
显示类型是EGA，则还分为整屏窗口移动和区域内窗口移动。这里首先处理显示卡是EGA/VGA 
显示类型的情况。
*/
	if (video_type == VIDEO_TYPE_EGAC || video_type == VIDEO_TYPE_EGAM)
	{
/* 如果移动起始行top=0，移动最底行bottom = video_num_lines = 25，则表示整屏窗口向下
 ...
 *	%0 - eax(擦除字符+属性)；
 *	%1 - ecx(top行下1行开始到bottom行所对应的内存长字数)；
 *	%2 - edi(top行所处的内存位置)；
 *	%3 - esi(top+l行所处的内存位置)。
*/
		} else {
...
/* 向下卷动一行。
 * 将屏幕滚动窗口向上移动一行，相应屏幕滚动区域内容向下移动1行。并在移动开始行的上方
 * 出现一新行。参见程序列表后说明。处理方法与scrup()相似，只是为了在移动显示内存数
 * 据时不会出现数据覆盖的问题，复制操作是以逆向进行的，即先从屏幕倒数第2行的最后一个
 * 字符开始复制到最后一行，再将倒数第3行复制到倒数第2行等等。因为此时对EGA/VGA显示
 * 类型和MDA类型的处理过程完全一样，所以该函数实际上没有必要写两段相同的代码。即这里
 * if和else语句块中的操作完全一样！
 */
static void scrdown(void)
{
/* 本函数也针对两类显示类型（EGA/VGA和MDA）分别进行操作。如果显示类型是EGA，则执行
 * 下列操作。（这里if和else中的操作完全一样！以后的内核就合二为一了。）	*/
	if (video_type == VIDEO_TYPE_EGAC || video_type == VIDEO_TYPE_EGAM)
	{
...
函数名称ri(reVerse index反向索引)是指控制字符RI或转义序列“ESC M”。
*/
static void ri (void)
{
	if (y > top)
...
/* 删除屏幕上与光标位置相关的部分。
 * ANSI控制序列：’ESC [ Ps J’（Ps =
 *	0 -删除光标处到屏幕底端；
 *	1 -删除屏幕开始到光标处；
 *	2 -整屏删除）。
 * 本函数根据指定的控制序列具体参数值，执行与光标位置相关的删除操作，并且在擦除字符或行
 * 时光标位置不变。函数名称csi—J (CSI - Control Sequence Introducer，即控制序
 * 列引导码)指明对控制序列“CSI Ps J”进行处理。
...
/* 设置显示字符属性。
ANSI转义序列：’ESC [Psm^Ps =0默认属性；1加粗；4加下划线；7反显；27正显。
...
/* 根据设置显示光标。根据显示内存光标对应位置pos，设置显示控制器光标的显示位置。	*/
static inline void
set_cursor (void)
{
	cli ();
/* 首先使用索引寄存器端口选择显示控制数据寄存器rl4(光标当前显示位置高字节)，然后
 * 写入光标当前位置高字节（向右移动9位表示高字节移到低字节再除以2）。是相对于默认
 * 显示内存操作的。再使用索引寄存器选择H5，并将光标当前位置低字节写入其中。
 */
	outb_p (14, video_port_reg);			/* 选择数据寄存器rl4。	*/
	outb_p (0xff & ((pos - video_mem_start) >> 9), video_port_val);
/* 再使用索引寄存器选择r15，并将光标当前位置低字节写入其中。	*/
	outb_p (15, video_port_reg);			/* 选择数据寄存器rl5。	*/
...
送85行上定义的应答序列（即’ESC [?l;2c’]来响应主机的序列，该序列告诉主机本终端
是具有高级视频功能的VT100兼容终端。处理过程是将应答序列放入读缓冲队列中，并使用
 copy—to—cooked()函数处理后放入辅助队列中。
*/
static void
respond (struct tty_struct *tty)
{
	...
	while (*p)
	{							/* 将字符序列放入写队列。	*/
		PUTCH (*p, tty->read_q);/* 逐字符放入。include/linux/tty.h，33 行。	*/
		p++;
	}
	sti ();						/* 开中断。	*/
	copy_to_cooked (tty);		/* 转换成规范模式(放入辅助队列中)。tty—io.c，145 行。	*/
}
...
/* 在光标位置处插入nr行。
 * ANSI转义字符序列：’ESC [PnL’。该控制序列在光标处插入1行或多行空行。操作完成后光
 * 标位置不变。当空行被插入时，光标以下滚动区域内的行向下移动。滚动出显示页的行就丢失。
 * 参数nr =转义字符序列中的参数Pn。
 */
 ...
 * 字符。然后循环删除光标处指定字符数nr。	*/
	...
	while (nr--)			/* 循环删除指定字符数nr。	*/
		delete_char ();
}

/* 删除光标处的nr行。
ANSI转义序列：’ESC [PnM’。该控制序列在滚动区域内，从光标所在行开始删除1行或多
...
/* 首先取得写缓冲队列中现有字符数nr，然后针对队列中的每个字符进行处理。在处理每个字
 * 符的循环过程中，首先从写队列中取一字符c，根据前面处理字符所设置的状态state分步
 * 骤进行处理。状态之间的转换关系为：
 * state =	0:初始状态，或者原是状态4;或者原是状态1，但字符不是’[’；
 * 			1:原是状态0，并且字符是转义字符ESC(0xlb = 033 = 27)。处理后恢复状态0。
 * 			2:原是状态1，并且字符是’[’；
...
*/
	...
	/* 则将光标移到下一行上。	*/
					...
					c=9;
				} else if (c==7)	/* 如果字符c是响铃符BEL(7)，则调用蜂鸣函数，是扬声器发声。	*/
					sysbeep();
				break;
			case 1:
				state=0;
				if (c=='[')			/* 如果字符c 是'['，则将状态state 转到2。	*/
					...
					respond(tty);
				else if (x=='7')	/* 如果字符c 是'7'，则保存当前光标位置。注意这里代码写错！应该是(c=='7')。	*/
					save_cur();
				else if (x=='8')	/* 如果字符c 是'8'，则恢复到原保存的光标位置。注意这里代码写错！应该是(c=='8')。	*/
					restore_cur();
				break;
/* 如果在状态1(是转义字符ESC)时收到字符’[’，则表明是一个控制序列引导码CSI，于是转到这里状态2来处理。	*/
			case 2:
/* 首先对ESC转义字符序列保存参数的数组par[]清零，索引变量npar指向首项，并且
 * 设置状态为3。若此时字符不是’？’，则直接转到状态3去处理，若此时字符是’？’，
 * 说明这个序列是终端设备私有序列，后面会有一个功能字符。于是去读下一字符，再
 * 到状态3处理代码处。否则直接进入状态3继续处理。	*/
...
/* 状态4是处理转义字符序列的最后一步。1^面几个状态的处理我们已经获得了转义字符
 ...
 * 第619行定义一个寄存器变量a，该变量将被保存在一个寄存器中，以便于高效访问和操作。
 * 若想指定存放的寄存器（如eax），则可写成“register unsigned char a asm(“ax”)；”。
*/
void con_init(void)
{
...
/* 首先根据setup.s程序取得的系统硬件参数（见本程序第39—47行）初始化几个本函数专用
 * 的静态全局变量。	*/
 ...
 * 端口号和显示寄存器数据端口号。如果原始显示模式等于7，则表示是单色显示器。	*/
 ...
 * 卡。因此初始显示类型为EGA单色。虽然EGA卡上有较多显示内存，但在单色方式下最多只
 * 能利用地址范围在0xb0000—0xb8000之间的显示内存。然后置显示器描述字符串为’EGAm’。
 * 并会在系统初始化期间显示器描述字符串将显示在屏幕的右上角。
 * 注意，这里使用了 bx在调用中断int 0x10前后是否被改变的方法来判断卡的类型。若BL在
...
/* 再判断显示卡类别。如果BX不等于0x10，则说明是EGA/VGA显示卡。此时实际上我们可以使
 * 用32KB显示内存（0xb8000 — 0xc0000），但该程序只使用了其中16KB显示内存。
 */
		if ((ORIG_VIDEO_EGA_BX & 0xff) != 0x10)
		{
...
/* 如果BX寄存器的值等于0x10，则说明是CGA显示卡。只使用8KB显示内存。	*/
		else
		{
 ...
 * 位置处。首先将显示指针display—ptr指到屏幕第1行右端差4个字符处（每个字符需2个
 ...
 * 允许响应键盘发出的IRQ1请求信号。最后复位键盘控制器以允许键盘开始正常工作。
*/
