...
* 不了一个合法除操作的结果时也会产生这个异常。20行上标号’—do—divide—error’实际上是
* C语言函数do_divide_error()编译后所生成模块中对应的名称。函数’do—divide—error’在
* traps.c中实现（第97行开始）。
*/
_divide_error:
...
/* 下行上的号表示调用操作数指定地址处的函数，称为间接调用。这句的含义是调用引起本次 
* 异常的C处理函数，例如do_divide_errorO等。第40行是将堆栈指针加8相当于执行两次pop 
* 操作，弹出（丢弃）最后入堆栈的两个C函数参数（32行和34行入栈的值），让堆栈指针重新 
* 指向寄存器fs入栈处。
*/
...
* 向量2，并执行标准中断应答周期，因此很节省时间。丽I通常保留为极为重要的硬件事件使用。
...
/* int3 -- 指令引发的中断，与硬件中断无关。该指令通常由调式器插入被调式程序的代码中。处理过程同—debug。*/
_int3:
	pushl $_do_int3
	jmp no_error_code
...
* 计算时，CPU会等待其操作完成。下面88行上OxTO是协处理端口，用于清忙锁存器。通过写 
...
1:	outb %al,$0xA0			/* 再向8259 从中断控制芯片发送EOI（中断结束）信号。	*/
	popl %eax
	jmp _coprocessor_error	/* _coprocessor_error 原来在本文件中，现在已经放到5.3 asm.s 程序	*/

/* 以下中断在调用时CPU会在中断返回地址之后将出错号压入堆栈，因此返回时也需要将出错号弹出（参见图5.3(b)）。
