...
void sys_sync (void);		/* it's really int  实际上是整型int (fs/buffer.c,44) */

/* 该函数用来显示内核中出现的重大错误信息，并运行文件系统同步函数，然后进入死循环一死机。
 ...
 * 等同于现在gcc 的函数属性说明：void panic(const char *s) —attribute— ((noreturn));
 */
volatile void
panic (const char *s)
{
