...
/* 返回日期和时间。以下返回值是-EN0SYS的系统调用函数均表示在本版本内核中还未实现。	*/
int
sys_ftime ()
{
	return -ENOSYS;
...
/* 设置当前任务的实际以及/或者有效组ID（gid）。如果任务没有超级用户特权，那么只能互换
其实际组ID 和有效组ID。如果任务具有超级用户特权，就能任意设置有效的和实际的组ID。保
留的gid（saved gid）被设置成与有效gid 同值。
*/
int
 ...
 * 则时间值也存储在那里。
 * 由于参数是一个指针，而其所指位置在用户空间，因此需要使用函数put—fS—long()来
 ...
 * 由于参数是一个指针，而其所指位置在用户空间，因此需要使用函数get—fs—longO来访问该
