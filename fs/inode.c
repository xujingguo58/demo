...
static void read_inode (struct m_inode *inode);		/* 读指定i节点号的i节点信息，294行。	*/
static void write_inode (struct m_inode *inode);	/* 写i节点信息到高速缓冲中，314行。	*/

/* 等待指定的i 节点可用。	*/
/* 如果i节点已被锁定，则将当前任务置为不可中断的等待状态，并添加到该i节点的等待队
 * 列i—wait中。直到该i节点解锁并明确地唤醒本任务。	*/
...
/* 对指定的i节点解锁。
 * 复位i节点的锁定标志，并明确地唤醒等待在此i节点等待队列i—wait上的所有进程。	*/
static inline void unlock_inode (struct m_inode *inode)
{
	inode->i_lock = 0;
	wake_up (&inode->i_wait);		/* kernel/sched.c，第 188 行。	*/
}
 ...
 * 为0。若是则显示警告信息。然后释放之，即把i节点的设备号字段i—dev置0。第48行上
 * 的指针赋值“0+inode—table”等同于“inode_table”、“&inode_table[0]”。
 * 不过这样写 II可能更明了一些。	*/
 ...
 * 然后设置i节点改变时间，置i节点已修改标志。最后返回逻辑块号。函数new—block()
 * 定义在bitmap.c程序中第75行开始处。	*/
	if (block < 7)
	{
 ...
 * 块号为0的话，则需申请一磁盘块（逻辑块）作为二次间接块的二级块i，并让二次间接块
 * 的一级块中第（block/512）项等于该二级块的块号i。然后置位二次间接块的一级块已
...
/* 如果是管道i 节点，则唤醒等待该管道的进程，引用次数减1，如果还有引用则返回。否则
 * 释放管道占用的内存页面，并复位该节点的引用计数值、已修改标志和管道标志，并返回。	
 * 对于pipe 节点，inode->i_size 存放着物理内存页地址。
 * 参见get_pipe_inode()，228，234 行。	*/
	if (inode->i_pipe)
	{
...
/* 如果是块设备文件的i节点，此时逻辑块字段0(i—zone[0])中是设备号，则刷新该设备。
 * 并等待i节点解锁。	*/
 ...
 * 释放），否则就说明i节点的引用计数值为1 (因为第155行已经判断过引用计数是否为零)。
...
struct m_inode * get_empty_inode (void)
{
	...
	do
	{
/* 在初始化last_inode指针指向i节点表头一项后循环扫描整个i节点表。如果last_inode已经
 * 指向i节点表的最后1项之后，则让其重新指向i节点表开始处，以继续循环寻找空闲i节点项。如
 * 果last_inode所指向的i节点的计数值为0，则说明可能找到空闲i节点项。让inode 指向该i
 * 节如果该i节点的已修改标志和锁定标志均为0，则我们可以使用该i节点，于是退出for循环。	*/
		inode = NULL;
		for (i = NR_INODE; i; i--)
		{
...
/* 如果i节点又被其他占用的话（i节点的计数值不为0 了），则重新寻找空闲i节点。否则
 ...
 * 的i—zone[0]和i—zone[l]中分别用来存放管道头和管道尾指针。最后设置i节点是管道i节
 * 点标志并返回该i节点号。	*/
...
/* 接着扫描i节点表了寻找参数指定节点号nr的i节点。并递增该节点的引用次数。如果当
 ...
 * 并从安装在此i节点上的文件系统超级块中取设备号，并令i节点号为R00T_IN0，即为1。
 ...
 * 用于计算逻辑块号的每块i节点数信息INODES—PER—BLOCK。在计算出i节点所在的逻辑块
 ...
 * 例如，节点号16的i节点结构应该在B=(16-l)/16 = 0的块上。这里我们从设备上读取该
