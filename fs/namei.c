...
/* 下而宏中右侧表达式是访问数组的一种特殊使用方法。它基于这样的一个事实，即用数组名和
数组下标所表示的数组项(例如a[b])的值等同于使用数组首指针(地址)加上该项偏移地址
的形式的值*(a+b)，同时可知项a[b]也可以表示成b[a]的形式。ld 此对于字符数组项形式
...
其ASCII码值来表示，方法是在字符的ASCII码值前而加一个反斜杠。例如字符”v"可以表示
成”\x76”或者”\166"。因此对于不可显示的字符(例如ASCII码值为0x00--Oxlf的控制字符)
就可用其ASCII码值来表示。
下而是访问模式宏。x是头文件include/fcntl.h中第7行开始定义的文件访问(打开)标志。
这个宏根据文件访问标志x的值来索引双引号中对应的数值。双引号中有4个八进制数值(实
际表示4个控制字符):"\004\002\006\377"，分别表示读、‘弓和执行的权限为:r,	w,	rw
和wxrwxrwx，并目‘分别对应x的索引值0--3。例如，如果x为2，则该宏返回八进制值006
表示可读可写Crw)。另外，其中0 l1CCNfOD1:二00003，是索引值x的屏蔽码。
...
/*
* permission()
* 该函数用于检测一个文件的读/写/执行权限。
* 我不知道是否只需检查euid，还是需要检查euid 和uid 两者，不过这很容易修改。
*/
/* 检测文件访问许可权限。	*/
/* 参数：inode - 文件对应的i 节点；	*/
/* mask - 访问属性屏蔽码。	*/
 ...
 * 返回：相同返回1，不同返回0。
 * 第65行上定义了一个局部寄存器变量same。该变量将被保存在eax寄存器中，以便于高效访问。
 */
static int match(int len,const char * name,struct dir_entry * de)
{
	register int same __asm__("ax");
/* 首先判断函数参数的有效性。如果目录项指针空，或者目录项i节点等于0，或者要比较的
 * 字符串长度超过文件名长度，则返回0。如果要比较的长度len小于NAME—LEN，但是目录项
 * 中文件名长度超过len，也返回0。
 * 第69行上对目录项中文件名长度是否超过len的判断方法是检测name[len]是否为NULL。
 ...
 *		%4 - ecx(比较的字节长度值len)。	*/
/* 清方向位。								*/	__asm__("cld\n\t"
/* 用户空间执行循环比较[esi++]和[edi++]操作，*/		"fs ; repe ; cmpsb\n\t"
/* 若比较结果一样(z=0)则设置al=1(same=eax)。*/		"setz %%al"
												:"=a" (same)
...
* 缓冲块以及目录项本身（作为一个参数-res—dir）。该函数并不读取目录项
* 的i节点-如果需要的话则自己操作。
*
* 由于有目录项，因此在操作期间也会对几种特殊情况分别处理-比如横越
* 一个伪根目录以及安装点。
*/
/* 查找指定目录和文件名的目录项。
* 参数：*dir -指定目录i节点的指针；name -文件名；namelen -文件名长度；
* 该函数在指定目录的数据（文件）中搜索指定文件名的目录项。并对指定文件名是的情
* 况根据当前进行的相关设置进行特殊处理。关于函数参数传递指针的指针的作用，请参
* 见linux/sched.c第151行前的注释。
* 返回：成功则函数高速缓冲区指针，并在抑es_dir处返回的目录项结构指针。失败则返回
* 空指针NULL。
*/
...
/* 同样，本函数一上来也需要对函数参数的有效性进行判断和验证。如果我们在前面第27行
 * 定义了符号常数NO—TRUNCATE，那么如果文件名长度超过最大长度NAME—LEN，则不予处理。
 * 如果没有定义过NO—TRUNCATE，那么在文件名长度超过最大长度NAME—LEN时截短之。	*/
#ifdef NO_TRUNCATE
	if (namelen > NAME_LEN)
		return NULL;
#else
 ...
 * 目录项结构指针。如果文件名长度等于0，则返回NULL，退出。	*/
	...
	*res_dir = NULL;
	if (!namelen)
		return NULL;
/* 接下来我们对目录项文￥￥是’..’的情况进行特殊处理。如果当前进程指定的根i节点就是
 * 函数参数指定的目录，贝B兑明对于本进程来说，这个目录就是它的伪根目录，即进程只能访
 * 问该目录中的项而不能后退到其父目录中去。也即对于该进程本目录就如同是文件系统的根
 * 目录。因此我们需要将文件名修改为
 * 否则，如果该目录的i节点号等于R00T_IN0(1号)的话，说明确实是文件系统的根i节点。
 ...
 * 存在i节点结构的i_zone[9]数组中。我们先取其中第1个块号。如果目录i节点指向的第
 ...
 * 我们就从节点所在设备读取指定的目录项数据块。当然，如果不成功，则也返回NULL退出。	*/
	...
	if (!(bh = bread ((*dir)->i_dev, block)))
		return NULL;
/* 此时我们就在这个读取M录i节点数据块中搜索匹配指定文件名的目录项。首先让de指
 ...
 * 跳过该块，继续读目录的下一逻辑块。若该块不空，就让de指向该数据块，然后在其中
 * 继续搜索。其中137行上i/DIR_ENTRIES—PER—BLOCK可得到当前搜索的目录项所在目录文
 * 件中的块号，而bmapO函数（inode.c，第142行）则可计算出在设备上对应的逻辑块号。	*/
 ...
 * 号保存在i节点结构的i_zone[9]数组中。我们先取其中第1个块号。如果目录i节点指向
 ...
 * 退出。另外，如果参数提供的文件名长度等于0，则也返回NULL退出。	*/
	...
	if (!(bh = bread (dir->i_dev, block)))
		return NULL;
/* 此时我们就在这个目录点数据块中循环查找最后未使用的空目录项。首先让目录项结构
 ...
 * 而create_blockO函数（inode.c，第145行）则可读取或创建出在设备上对应的逻辑块。	*/
		if ((char *) de >= BLOCK_SIZE + bh->b_data)
		{
			brelse (bh);
			bh = NULL;
...
/* 本函数执行不到这里。这也许是Linus在写这段代码时，先复制了上面Hnd—entryO函数
 * 的代码，而后修改成本函数的。	*/
	brelse (bh);
	return NULL;
}
...
/*
* get_dir()
* 该函数根据给出的路径名进行搜索，直到达到最顶端的目录。如果失败则返回NULL。
*/
 ...
 * 此时变量inode指向了正确的i节点一进程的根i节点或当前工作目录i节点之一。	*/
...
/* /然后针对路径名中的各录名部分和文件名进行循环处理。首先把得到的i节点引用计数
 * 增1，表示我们正在使用。在循环处理过程中，我们先要对当前正在处理的目录名部分（或
 * 文件名）的i节点进行有效性判断，并且把变量thisname指向当前正在处理的目录名部分
 ...
 * 搜索检测字符，直到字符是一个结尾符（NULL）或者是一个’/’字符。此时变量namelen正
 ...
 * 会返回该最后目录的i节点！例如：对于路径名/usr/src/linux，该函数将只返回src/目录
 * 名的i节点。	*/
		...
		if (!c)
			return inode;
/* 在得到当前目录名部分（或文件名）后，我们调用查找目录项函数find—entryO在当前处
 ...
 * 环处理路径名中的下一目录名部分（或文件名）。	*/
		...
		brelse (bh);
		iput (inode);
		if (!(inode = iget (idev, inr)))	/* 取 i 节点内容。	*/
	return NULL;
	}
}
 ...
 * 返回src/目录名的i节点。因为函数dir—nameiO把不以’/’结束的最后一个名字当作一个
 * 文件名来看待，所以这里需要单独对这种情况使用寻找目录项i节点函数find—entryO进行
 ...
 * 然后取对应节点号的i节点，修改其被访问时间为当前时间，并置已修改标志。最后返回该i
 * 节点指针。	*/
	inr = de->inode;
	dev = dir->i_dev;
	brelse (bh);
 ...
 * S—IRUSR (用户具有读文件权限)、S—IRWXG(组成员具有读、写和执行权限)等等。对于新
 ...
 * 0_TRUNC却置位了，则在文件打开标志中添加只写标志0_WR0NLY。这样做的原因是由于截零
 * 标志0_TRUNC必须在文件可写情况下才有效。然后使用当前进程的文件访问许可屏蔽码，屏
 * 蔽掉给定模式中的相应位，并添上普通文件标志I_REGULAR。该标志将用于打开的文件不存
 * 在而需要创建文件时，作为新文件的默认属性。参见下面360行上的注释。	*/
...
/* 若上面（360行）在目录中取文件名对应目录项结构的操作成功（即bh不为NULL），则说
 * 明指定打开的文件已经存在。于是取出该目录项的i节点号和其所在设备号，并释放该高速
 * 缓冲区以及放回目录的i节点。如果此时独占操作标志0_XCL置位，但现在文件已经存在，
 * 则返回文件已存在出错码退出。	*/
	inr = de->inode;
	dev = dir->i_dev;
	brelse (bh);
 ...
 * 块指针为NULL），贝輸回目录的i节点；把所申请的i节点引用连接计数复位，并放回该
 * i节点，返回出错码退出。	*/
	bh = add_entry (dir, basename, namelen, &de);
	if (!bh)
	{
 ...
 * 1 个直接块没有指向任何磁盘块号，或者该直接块读不出，则显示警告信息"设备dev上目
 * 录错"，返回 0(失败)。	*/
 ...
 * 对于第 1 个目录项（”.”），它的 i 节点号字段 LQRGH 应该等于当前目录的 i 节点号。对于
 * 第 2 个目录项（”..”），它的 i 节点号字段 LQRGH 应该等于上一层目录的 i 节点号，不会
 ...
 * 文件中下一块含有目录项的磁盘块。读取的方法是根据当前检测的目录项序号nr计算出对
...
/* 然后根据指定目录的i节点和目录名利用函数find—entryO寻找对应目录项，并返回包含该
 ...
 * 回包含要删除目录名的目录i节点和该要删除目录的i节点，然后释放高速缓冲区，返回
 * 出错码。	*/
...
/* 然后根据指定目录的i节点和目录名利用函数find_entryO寻找对应目录项，并返回包含该
 * 目录项的缓冲块指针bh、包含该目录项的目录的i节点指针dir和该目录项指针de。再根据
 * 该目录项de中的i节点号利用iget()函数得到对应的i节点inode。如果对应路径名上最
 * 后目录名的目录项不存在，则释放包含该目录项的高速缓冲区，放回目录的i节点，返回文
 * 件已经存在出错码，并退出。如果取目录项的i节点出错，则放回目录的i节点，并释放含
 ...
 * 件也将被删除，并释放所占用的设备空间。参见fs/inode.c，第180行。	*/
	inode->i_nlinks--;
	inode->i_dirt = 1;
