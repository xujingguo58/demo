...
*/
/*
* ‘buffer.c’ 用于实现缓冲区高速缓存功能。 通过不让中断过程改变缓冲区， 而是让调
* 用者来执行，避免了竞争条件（当然除改变数据以外）。	注意！由于中断可以唤醒一个调
* 用者，因此就需要开关中断指令（cli-sti）序列来检测等待调用返回。* 但需要非常地快
* (我希望是这样)。
*/
/*
...
*该程序最好的地方了，因为它需要使己更换软盘缓冲失效。
*/

#include <stdarg.h>		/* 标准参数头文件。以宏的形式定义变量参数列表。主要说明了-个
						 * 类型(va_list)和三个宏(va_start, va_arg 和va_end)，
						 * 用于vsprintf、vprintf、vfprintf 函数。	*/

#include <linux/config.h>	/* 内核配置头文件。定义键盘语言和硬盘类型（HD_TYPE）可选项。	*/
#include <linux/sched.h>	/* 调度程序头文件，定义了任务结构task_struct、初始任务0 的数
							 * 据，还有一些有关描述符参数设置和获取的嵌入式汇编函数宏语句。	*/

#include <linux/kernel.h>	/* 内核头文件。含有一些内核常用函数的原形定义。	*/
#include <asm/system.h>		/* 系统头文件。定义了设置或修改描述符/中断门等的嵌入式汇编宏。	*/
#include <asm/io.h>			/* io 头文件。定义硬件端口输入/输出宏汇编语句。	*/

/* 变量end是由编译时的连接程序Id生成，用于表明内核代码的末端，即指明内核模块末端
 * 位置，参见图12-15。也可以从编译内核时生成的System.map文件中查出。这里用它来表
 * 明高速缓冲区开始于内核代码末端位置。
 * 第33行上的buffer_wait变量是等待空闲缓冲块而睡眠的任务队列头指针。它与缓冲块头
 * 部结构中b—wait指针的作用不同。当任务申请一个缓冲块而正好遇到系统缺乏可用空闲缓
...
/* 下面定义系统缓冲区中含有的缓冲块个数。这里，NR—BUFFERS是一个定义在linux/fs.h头
 * 文件第34行的宏，其值即是变量名nr_buffers，并且在fs.h文件第166行声明为全局变量。
...
/* 设备数据同步。
 * 同步设备和内存高速缓冲中数据。其中，sync—inodes()定义在inode.c，59行。
 */
int sys_sync (void)
{
	int i;
 ...
 * 已修改过（b—dirt标志置位），若是就对其执行写盘操作。因为在我们睡眠期间该缓冲块 
...
* 所以仅在执行'mount'或'open'时才调用它。
* 我想这是将速度和实用性相结合的
* 最好方法。
* 若在操作过程当中更换软盘，会导致数据的丢失，这是咎由自取?。
*
 ...
 * 测试软盘是否已更换，如果没有则退出。floppy—change()在blk—drv/floppy.c第139行。	*/
 ...
 * 设备的i节点和数据块信息所占踞的高速缓冲块无效。	*/
	...
	if (!floppy_change (dev & 0x03))
		return;
/* 软盘已经更换，所以释放对应设备的i 节点位图和逻辑块位图所占的高速缓冲区；
 ...
 * b—prev赋bh值。即第163行前应该增加判断“if (bh——>b_next]”。该错误到0.96版后
 * 才被纠正。
 */
	bh->b_prev = NULL;
	bh->b_next = NULL;
	...
	bh->b_next->b_prev = bh;			/* 此句前应添加 “if (bh-)b—next)” 判断。	*/
}
...
/* 在高速缓冲中寻找给定设备和指定块的缓冲区，如果没有找到则返回NULL，退出。	*/
			...
			if (!(bh = find_buffer (dev, block)))
				return NULL;
/* 对该缓冲块增加引用计数，并等缓冲块解锁（如果已被上锁）。由于经过了睡眠状态，
...
/* OK，最终我们知道该缓冲区是指定参数的唯一一块，而且还没有被使用
 * (b_count=0)，未被上锁(b_lock=0)，并且是干净的（未被修改的）*/
 ...
 * 则返回。	*/
	...
	if (bh->b_uptodate)
		return bh;
/* 否则我们就调用底层块设备读写11—^blockO函数，产生读设备块请求。然后等待指定
 * 数据块被读入，并等待缓冲区解锁。在睡眠醒来之后，如果该缓冲区已更新，贝彳返回缓冲
 ...
 * 的数组。该函数仅用于mm/memory.c文件的do—no—page()函数中（第386行）。
*/
void
...
*/
/*
* OK，breada 可以象bread 一样使用，但会另外预读一些块。
* 该函数参数列表
* 需要使用一个负数来表明参数列表的结束。
*/
 ...
 * 块。如果该缓冲块数据无效（更新标志未置位），则发出读设备数据块请求。	*/
	...
	if (!bh->b_uptodate)
		ll_rw_block (READ, bh);
/* 然后顺序取可变参数表中其彳Hi卖块号，并作与上面同样处理，但不引用。注意，336行上
 * 有一个bug。其中的bh应该是tmp。这个bug直到在0.96版的内核代码中才被纠正过来。
 * 因为这里是预读随后的数据块，只需读进高速缓冲区但并不是马上就使用，所以第337行语
 * 句需要将其引用计数递减释放掉该块（因为getblkO函数会增加引用计数值）。	*/
...
/* 缓冲区初始化函数。
 * 参数buffer—end是缓冲区内存末端。对于具有16MB内存的系统，缓冲区末端被设置为4MB。
 * 对于有8MB内存的系统，缓冲区末端被设置为2MB。该函数从缓冲区开始位置start_buffer
 * 处和缓冲区末端buffer—end处分别同时设置（初始化）缓冲块头结构和对应的数据块。直到
 ...
 * 的结构buffer—head，并将这些buffer—head组成双向链表。
...
/* 最后初始化hash 表（哈希表、散列表），置表中所有的指针为NULL。	*/
	for (i = 0; i < NR_HASH; i++)
		hash_table[i] = NULL;
}
