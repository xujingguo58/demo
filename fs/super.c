...
/* set—bitO使用了 setb指令，因为汇编编译器gas不能识别指令setc */
/* 测试指定位偏移处比特位的值，并返回该原比特位值（应该取名为test—bitO更妥帖）。
 * 嵌入式汇编宏。参数bitnr是比特位偏移值，addr是测试比特位操作的起始地址。
 * %0 - ax(――res)，%l - 0，%2 - bitnr，%3 - addr
 ...
 * 位标志CF设置操作数%al。如果CF = 1贝彳%al = 1，否则%al = 0。	*/
#define set_bit(bitnr,addr) ({ \
register int __res __asm__( "ax"); \
__asm__( "bt %2,%3;setb %%al": "=a" (__res): "a" (0), "r" (bitnr), "m" (*(addr))); \
__res; })

struct super_block super_block[NR_SUPER];	/* 超级块结构表数组（NR—SUPER = 8）。	*/
/* this is initialized in init/main.c */
/* ROOT_DEV 已在init/main.c 中被初始化 */
int ROOT_DEV = 0;							/* 根文件系统设备号。	*/

/* 以下 3 个函数（lock_super()、free_super()和 wait_on_super()）的作用与
 * inode.c 文件中头3个函数的作用雷同，只是这里操作的对象换成了超级块。
 * 锁定超级块。
...
/* 首先判断参数给出设备的有效性。若设备号为0则返回空指针。然后让3指向超级块数组起始
 * 处，开始搜索整个超级块数组，以寻找指定设备dev的超级块。第62行上的指针赋 值语句
 *"s = 0+super—block"等同于 “s = super—block"、"s = &super—block[0]"。	*/
	if (!dev)
		return NULL;
 ...
 * 如果是则返回该超级块的指针。
 * 否则就重新对超级块数组再搜索一遍，因此s 重又指向超级块数组开始处。 */
		if (s->s_dev == dev)
		{
			wait_on_super (s);
			if (s->s_dev == dev)
 ...
 * 面选定的超级块数组中的项（即置s—dev=0），并解锁该项，返回空指针退出。否则就将设
 ...
 * 保存在设备上2号块开始的逻辑块中，共占用s—imap—blocks个块。逻辑块位图在i节点位
 * 图所在块的后续块中，共占用s—zmap—blocks个块。	*/
 ...
 * 则不能被卸载，返回忙出错号。	*/
	...
	if (dev == ROOT_DEV)
		return -EBUSY;
/* 如果在超级块表中没有设备上文件系统的超级块，或者已找到但是该设备上文件系统
 ...
 * 件，如果有则返回忙出错码。	*/
	...
	for (inode = inode_table + 0; inode < inode_table + NR_INODE; inode++)
		if (inode->i_dev == dev && inode->i_count)
			return -EBUSY;
/* 现在该设备上文件系统的卸载条件均得到因此我们可以开始实施真正的卸载操作了。
 ...
 * 为空，并放回设备文件系统的根i节点，接着置超级块中被安装系统根i节点指针为空。	*/
	...
	iput (sb->s_isup);
	sb->s_isup = NULL;
/* 最后我们释放该设备I超级块以及位图占用的高速缓冲块，并对该设备执行高速缓冲与设
...
/* 安装文件系统（系统调用）。
 * 参数dev—name是设备文件名，dir—name是安装到的目录名，rw—flag被安装文件系统的可
 ...
 * 号在其i节点的i—zone[0]中。另外，由于文件系统必须在块设备中，因此如果不是块设备
 * 文件，则放回刚取得的i节点dev—i，返回出错码。	*/
...
/* 0K，现在上面为了得到设备号而取得的i节点dev_i已完成了它的使命，因此这里放回该设
 ...
 * 败，则放回该安装点i节点dir—i并返回出错码。一个文件系统的超级块会首先从超级块表
 ...
 * 并设置安装位置i节点的安装标志和节点已修改标志。然后返回0(安装成功)。
	*/
	...
	dir_i->i_dirt = 1;	/* NOTE! we don't iput(dir_i)*/	/*注意！这里没用 iput(dir—i)*/
	return 0;			/* we do that in umount */		/* 这将在umount 内操作 */
}
 ...
 * 设备上超级块失败或取根节点失败，则都显示信息并停机。	*/
	...
	if (!(mi = iget (ROOT_DEV, ROOT_INO)))		/* 在 fs.h 中 R00T—IN0 定义为 1。	*/
 ...
 * 计出空闲块数。这里宏函数set_itO只是在测试比特位，而非设置比特位。"i&8191"用于
 ...
 * 磁盘块包含的比特位数。	*/
	...
	while (--i >= 0)
		if (!set_bit (i & 8191, p->s_zmap[i >> 13]->b_data))
			free++;
/* 在显示过设备上空闲逻逻辑块总数之后。我们再统计设备上空闲i节点数。首先令i
